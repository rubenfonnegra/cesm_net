...
## 
def sample_images(args, dataloader, models, epoch, difference = True, output_dir = None, shuffled = True, write_log = False, img_complete=False):
...
def plot_imgs(args, lucky, dataloader, output_dir, models, difference=True, output_attn=None, attention = False, write_log = False):
    ...
    for k, l in tqdm(enumerate(lucky), ncols=100):
        ...
        out_LE, skip_LE = encoder(real_in)
        fake_out        = decoder(out_LE, skip_LE)
...
def load_model(args):
    ...
    if args.model == "VAE":
        encoder_1   = Encoder_VAE(args.channels)
        encoder_2   = Encoder_VAE(args.channels)
        decoder     = Decoder_VAE()
