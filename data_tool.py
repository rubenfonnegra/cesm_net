...
def extract_patches (images, n_patches = 25, patch_size = 256, return_patch_locs=False, porcent_bg = 10):
    #
    # n_patches = 25
    # patch_size = 256
    
    """ Calcular el numero de parches correspondiente a cada uno """
    n_patches_black_bg = (n_patches * porcent_bg) // n_patches
    n_patches_without_bg = n_patches - n_patches_black_bg
    
    im_1, im_2 = images
    locs = np.array(np.where(im_1 > 0)).T
    
    patches_im1, patches_im2, patches_bg_im1, patches_bg_im2 = [], [], [], []
    im_1_c = im_1.copy() 
    im_2_c = im_2.copy()
    np.random.seed(0)
    i = 0
    ...
    while ((len(patches_im1) < n_patches_without_bg) and (len(patches_bg_im1) < n_patches_black_bg)) or (i<2000):
        #
        i += 1
        ...
        if (porcent_zero <= 0.4 ) and (porcent_zero >= 0.15) and (len(patches_bg_im1) < n_patches_black_bg):
...
parser.add_argument("--porcent", type=int)
...
for i in tqdm(meta_.index, ncols = 100):
    ...
    patches_le, patches_rc, im_1_c, im_2_c = extract_patches ([dcm1, dcm2], n_patches = 100, patch_size = 256, return_patch_locs=True, porcent_bg=porcent)
